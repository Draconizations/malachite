...
```
The passage passed to `x-frame` will be rendered on the frame's initialization, unless another passage is already pressent in the [State](../general/state)
...
All `x-frame` directives reference the [unnamed frame](../general/frames#unnamed-frame) by default. To use a different frame, you can specify a name.
```html
